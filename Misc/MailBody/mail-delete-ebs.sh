...
email_content=$(echo "$resource" | \
  ...
  awk -F, -v tz="America/New_York" -v account_id="$account_id" '
    BEGIN {
      print "<html><head><title>AWS Unattached EBS Volumes To Be Deleted </title></head><body>"
      print "<h2 style=\"font-family:Arial, sans-serif; color:red;\">AWS Unattached EBS Volumes To Be Deleted </h2>"
      print "<p style=\"font-family:Arial, sans-serif; color:#555;\"><b>The following EBS volumes are not attached to any EC2 instances and marked for delete,please approve to delete these volumes:</b></p>"
    }
    {
      vol_id=""
      create_time=""
